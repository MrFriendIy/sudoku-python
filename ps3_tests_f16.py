# test2 suite for Problem Set 3 (Simulating Robots)
# Fall 2016
...
import imp
import test2
# test2 = imp.load_compiled("test2.py")
import ps3
...
class ps3_P1A(unittest.TestCase):
    """test2 the RectangularRoom abstract base class"""    
    def test2_unimplemented_methods(self):
        """test2 if student implemented methods in RectangularRoom abstract class that should not be implemented"""
        room = ps3.RectangularRoom(2,2,1)
        self.assertRaises(NotImplementedError, room.get_num_tiles)
        pos = test2.Position(1,1)
    ...
    def test2_room_dirt_dirty(self):
        """ 
    ...
    def test2_room_dirt_clean(self):
        """ 
        Can fail either because get_dirt_amount is working incorrectly 
        OR the student is initializing the dirt amount incorrectly
        """
    ...
    def test2_is_tile_cleaned_dirty(self):
        """ test2 is_tile_cleaned"""
        width, height, dirt_amount = (3, 4, 1)
        room = ps3.RectangularRoom(width, height, dirt_amount)
    ...
    def test2_is_tile_cleaned_clean(self):
        """ test2 is_tile_cleaned"""
        width, height, dirt_amount = (3, 4, 0)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        # Check all squares are unclean at start, given initial dirt > 1
    ...
    def test2_clean_tile_at_position_PosToZero(self):
        """ test2 if clean_tile_at_position removes all dirt"""
        width, height, dirt_amount = (3, 4, 1)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        # Clean the tiles and confirm they are marked as clean
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(test2.Position(x + random.random(), y + random.random()), dirt_amount) 
    ...
    def test2_clean_tile_at_position_PosToPos(self):
        """ test2 if clean_tile_at_position removes all dirt"""
        width, height, dirt_amount = (3, 4, 2)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        # Clean the tiles and confirm they are marked as clean
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(test2.Position(x + random.random(), y + random.random()), dirt_amount - 1) 
                # using random.random in case there is any issue with specific parts of a tile
        for x, y in xyrange(width, height):
            self.assertFalse(room.is_tile_cleaned(x, y),
    ...
    def test2_clean_tile_at_position_ZeroToZero(self):
        """ test2 if clean_tile_at_position removes all dirt"""
        width, height, dirt_amount = (3, 4, 0)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        # Clean the tiles and confirm they are marked as clean
        for x, y in xyrange(width, height):
            room.clean_tile_at_position(test2.Position(x + random.random(), y + random.random()), 1) 
                # using random.random in case there is any issue with specific parts of a tile
        for x, y in xyrange(width, height):
            self.assertTrue(room.is_tile_cleaned(x, y),
    ...
    def test2_get_num_cleaned_tiles_FullIn1(self):
        "test2 get_num_cleaned_tiles for cleaning subset of room completely with 1 call"
        width, height, dirt_amount = (3, 4, 1)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        cleaned_tiles = 0
        # Clean some tiles
    ...
    def test2_get_num_cleaned_tiles_Partial(self):
        "test2 get_num_cleaned_tiles for cleaning subset of room incompletely"
        width, height, dirt_amount = (3, 4, 2)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        cleaned_tiles = 0
        # Clean some tiles
    ...
    def test2_get_num_cleaned_tiles_FullIn2(self):
        """test2 get_num_cleaned_tiles for cleaning subset of room in two calls"""
        width, height, dirt_amount = (3, 4, 2)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        cleaned_tiles = 0
        # Clean some tiles
    ...
    def test2_get_num_cleaned_tiles_OverClean(self):
        "test2 cleaning already clean tiles does not increment counter"
        width, height, dirt_amount = (3, 4, 2)
        room = ps3.RectangularRoom(width, height, dirt_amount)
    ...
    def test2_is_position_in_room(self):
        "test2 is_position_in_room"
        width, height, dirt_amount = (3, 4, 2)
        room = ps3.RectangularRoom(width, height, dirt_amount)
        solution_room = test2.RectangularRoom(width, height, dirt_amount)

        for x in [0.0, -0.1, width - 0.1, width, width + 0.1]:
            for y in [0.0, -0.1, height - 0.1, height, height + 0.1]:
                pos = test2.Position(x, y)
...
class ps3_P1B(unittest.TestCase):
    """test2 the Robot abstract base class"""
    def test2_unimplemented_methods(self):
        """test2 if student implemented methods in Robot abstract class that should not be implemented"""
        room = test2.EmptyRoom(2,2,1)
        robot = ps3.Robot(room,1,1)
        self.assertRaises(NotImplementedError, robot.update_position_and_clean)
    
    def test2_getset_robot_direction(self):
        """test2 get_robot_direction and set_robot_direction"""
        # instantiate EmptyRoom from solutions for test2ing
        width, height, dirt_amount = (3, 4, 2)
        solution_room = test2.EmptyRoom(width, height, dirt_amount)
...
class ps3_P2_ER(unittest.TestCase):
    """test2 the EmptyRoom subclass"""
    def test2_get_random_position(self):
        """test2 get_random_position
        ...
        sol_room = test2.EmptyRoom(width, height, dirt_amount)
        freq_buckets = {}
        for i in range(50000):
            pos = room.get_random_position()
            # confirm from test2 that this is a valid position
    ...
    def test2_get_num_tiles(self):
        """ test2 get_num_tiles method"""
        for i in range(10):        
    ...
    def test2_is_position_valid(self):
        """ test2 is_position_valid
...
class ps3_P2_FR(unittest.TestCase):                  
    """test2s the FurnishedRoom subclass """
    def test2_is_tile_furnished(self):
        """ test2 is_tile_furnished """        
        for trial in range(5):
            ...
            sol_room = test2.FurnishedRoom(width, height, dirt_amount)
    ...
    def test2_is_position_furnished(self):
        """ test2_is_position_furnished """        
        for trial in range(5):
            width, height, dirt_amount = (random.randint(2, 8), random.randint(2, 8), 1)
            # create room using student's class, set furniture tiles for solution class
            room = ps3.FurnishedRoom(width, height, dirt_amount)
            ...
            sol_room.furniture_tiles = room.furniture_tiles 
            for x,y in xyrange(width,height):
                pos = test2.Position(x + random.random(), y + random.random())
    ...
    def test2_get_num_tiles(self):
        """ test2 get_num_tiles method
            should refactor - is mostly copy of EmptyRoom test2        
        """
        for i in range(10):        
    ...
    def test2_get_random_position(self):
        """test2 get_random_position for FurnishedRoom
           test2s for validity of positions - could add distribution checking similar to empty room
        """
        width, height, dirt_amount = (5, 10, 1)
        # instanciate student's room
        room = ps3.FurnishedRoom(width, height, dirt_amount)
...
class ps3_P3(unittest.TestCase):
    """This  test2s EmptyRoom and Standard robot in various ways"""
    def createRoomAndRobots(self, num_robots):
        r = ps3.EmptyRoom(5, 7, 1)
        robots = [ps3.StandardRobot(r, 1.0, 1) for i in range(num_robots)]
        return r, robots

    def test2_BoundaryConditions(self):
        "test2 strict inequalities in random positions for the EmptyRoom and StandardRobot"
        for m in range(7000):
    ...
    def test2Robot(self):
        ...
        pos_buckets = {}
        dir_buckets = {}
        skip_pos_distribution_test2 = False
        for m in range(7000):
            r, robots = self.createRoomAndRobots(4)
            for r in robots:
                p = r.get_robot_position()
        ...
        # test2 that positions are correctly distributed
        if not skip_pos_distribution_test2:
        ...
        # test2 that directions are correctly distributed
        for t in range(36):
    ...
    def test2_update_position_and_cleanStandardRobot(self):
        "test2 StandardRobot.update_position_and_clean"
        r = ps3.EmptyRoom(3, 5, 1)
        robot = ps3.StandardRobot(r, 1.0, 1)
        robot.set_robot_position(test2.Position(1.5, 2.5))
        ...
        correctPos = test2.Position(2.5, 2.5)
...
"""The SIMULATION_TIME_LIMIT, SimulationThread class and Simulationtest2er class
    are all designed to help test2 various simulations for problems 4 and 5
    
    Right now there is no test2ing for the FurnishedRoom simulation, because the 
    ...
    and passes the test2s for StandardRobot and EmptyRoom, there should not be any
    additional issue.
"""
...
class Simulationtest2er(unittest.TestCase):
    def run_simulation(self, bounds, parameters):
        """
        test2s ps3.run_simulation.  The number of time-steps
        ...
        # Set daemon flag, so we can quit the test2 even if simulation threads
        # are still running
        thr.setDaemon(True)
        thr.start()
        # Allow SIMULATION_TIME_LIMIT seconds for test2 to finish
...
class ps3_P5_Standard(Simulationtest2er):
    """test2 the simulation time cleaning the EmptyRoom with a StandardRobot"""
    def test2Simulation1(self):
        "test2 cleaning 100% of a 5x5 room"
        try:
            self.run_simulation(((142, 173),), (1, 1.0, 1, 5, 5, 1, 1.0, 100, ps3.StandardRobot))
        except:
            print ("Unexpected error:", sys.exc_info()[1])
            raise
    def test2Simulation2(self):
        "test2 cleaning 75% of a 10x10 room (Standard Robot)"
        self.run_simulation(((183, 198),), (1, 1.0, 1, 10, 10, 1, 0.75, 100, ps3.StandardRobot))
    def test2Simulation3(self):
        "test2 cleaning 90% of a 10x10 room (Standard Robot)"
        self.run_simulation(((298, 327),), (1, 1.0, 1, 10, 10, 1, 0.9, 100, ps3.StandardRobot))
    def test2Simulation4(self):
        "test2 multiple robots (95% of a 20x20 room with 5 robots (Standard Robot))"
        self.run_simulation(((289, 303),), (5, 1.0, 1, 20, 20, 1, 0.95, 100, ps3.StandardRobot))
    def test2Simulation5(self):
        "test2 different speeds (90% of a 5x20 room with a robot of speed 0.2 (Standard Robot))"
        self.run_simulation(((1095, 1228),), (1, 0.2, 1, 5, 20, 1, 0.9, 100, ps3.StandardRobot))
    def test2Simulation6(self):
        "test2 multiple robots and different speeds (90% of a 10x10 room with 3 robots of speed 0.5 (Standard Robot))"
        self.run_simulation(((155, 180),), (3, 0.5, 1, 10, 10, 1, 0.9, 100, ps3.StandardRobot))
    # new test2s below here
    def test2Simulation7(self):
         "test2 cleaning 100% of a 5x5 room (Standard Robot, 5 dirt/tile, capcity = 3)"
         self.run_simulation(((206, 266),(180, 240)), (1, 1.0, 3, 5, 5, 5, 1.0, 100, ps3.StandardRobot))
    def test2Simulation8(self):
        "test2 cleaning 100% of a 5x5 room (Standard Robot, 6 dirt/tile, capacity = 3)"
        self.run_simulation(((206, 266),(180, 240)), (1, 1.0, 3, 5, 5, 6, 1.0, 100, ps3.StandardRobot))
    def test2Simulation9(self):
        """test2 different speeds (90% of a 3x10 room with a robot of speed 0.2 (Standard Robot)),
        capacity = 2, 6 dirt/tile"""
        self.run_simulation(((387, 447),(384, 444)), (1, 0.2, 2, 3, 10, 6, 0.9, 100, ps3.StandardRobot))
    def test2Simulation10(self):
        "test2 multiple robots (95% of a 10x10 room with 5 robots (Standard Robot)) capacity = 2, 6 dirt/tile"
        self.run_simulation(((137, 198),(130, 190)), (5, 1.0, 2, 10, 10, 6, 0.95, 100, ps3.StandardRobot))
    def test2Simulation11(self):
        """test2 multiple robots and different speeds (90% of a 5x5 room with 3 robots of speed 0.5
        (Standard Robot)), capacity = 2, 6 dirt/tile"""
        self.run_simulation(((48, 108), (45, 104)), (3, 0.5, 2, 5, 5, 6, 0.9, 100, ps3.StandardRobot))

class ps3_P5_Faulty(Simulationtest2er):
    """test2 the simulation time cleaning the EmptyRoom with a FaultyRobot"""
    def test2Simulation1(self):
        "test2 cleaning 100% of a 5x5 room with FaultyRobot"
        x = ps3.run_simulation(1, 1.0, 1, 5, 5, 1, 1.0, 100, ps3.FaultyRobot)
        self.assertTrue(125 <= x <= 275,
                        "Simulation output was outside of 99.7% confidence interval!\n")    
    def test2Simulation2(self):
        "test2 cleaning 75% of a 10x10 room with FaultyRobot"
    ...
    def test2Simulation3(self):
        "test2 cleaning 90% of a 10x10 room with FaultyRobot"
